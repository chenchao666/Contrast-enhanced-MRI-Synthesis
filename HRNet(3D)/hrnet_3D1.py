...
from keras.layers import UpSampling3D, add, concatenate, Lambda
...
def stem_net(input):
    x = Conv3D(128, kernel_size=(3,3,3),padding='same')(input)
    x = BatchNormalization(axis=-1)(x)
    x = Activation('relu')(x)
...
def fuse_layer3(x):
    ...
    x0 = concatenate([x0_0, x0_1],axis=-1)  #axis=-1
    return x0
...
def HRnet(batch_size, depth, height, width, channel, classes):
    ...
    ADC = SliceTensor(2)(inputs)
    ADC = Lambda(lambda x: K.expand_dims(x,-1))(ADC)
    T1 = stem_net(T1)
    T2 = stem_net(T2)
    ADC = stem_net(ADC)
    x = concatenate([T1,T2,ADC],axis=-1)
#    x = stem_net(inputs)
